...
// ============================================================================
// Optional<T> (FinalModel - inline format)
// ============================================================================

/// FinalModel for optional<T> (inline format, no pointers)
/// 
/// Format:
/// - 1 byte: has_value flag (0 = null, 1 = has value)
/// - If has_value: followed by inline value data
pub struct FinalModelOptional<'a, T, M>
where
    M: FinalModel,
{
    buffer: &'a [u8],
...
impl<'a, T, M> FinalModelOptional<'a, T, M>
where
    M: FinalModel,
{
    #[inline]
...
impl<'a, T, M> FinalModel for FinalModelOptional<'a, T, M>
where
    M: FinalModel,
{
    fn offset(&self) -> usize {
    ...
    fn extra(&self) -> usize {
        0 // Inline format, no extra data
    }
}

/// Mutable version for writing
pub struct FinalModelOptionalMut<'a, T, M>
where
    M: FinalModel,
{
    buffer: &'a mut crate::buffer::WriteBuffer,
    offset: usize,
    value_model_fn: fn(&'a mut crate::buffer::WriteBuffer, usize) -> M,
    _phantom: std::marker::PhantomData<T>,
}

impl<'a, T, M> FinalModelOptionalMut<'a, T, M>
where
    M: FinalModel,
{
    #[inline]
...
impl<'a, T, M> FinalModel for FinalModelOptionalMut<'a, T, M>
where
    M: FinalModel,
{
    fn offset(&self) -> usize {
